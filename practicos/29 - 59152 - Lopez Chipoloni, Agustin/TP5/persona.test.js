...
describe("Crear una persona", () => {
    ...
    test("1. Crear una persona (retorna codigo)", async () => {
        const personas = { nombre: "Juan", apellido: "Perez", edad: 30 }
        const res = await request(app).put("/personas").send(personas)
    ...
    test("3. Crear una persona (debe haber uno mas)", async () => {
        ...
        const crear = await request(app).put("/personas").send(personas)
...
describe("Actualizar una persona", () => {
    test("1. Actualizar una persona (retorna codigo)", async () => {
        const personas = { id: 2, nombre: "Juan", apellido: "Perez", edad: 30 }
        const res = await request(app).put("/personas").send(personas)
        expect(res.statusCode).toBe(201)
    })

    test("2. Si no existe la persona, no se actualiza", async () => {
        const personas = { id: 100, nombre: "Juan", apellido: "Perez", edad: 30 }
        const res = await request(app).put("/personas").send(personas)
        expect(res.statusCode).toBe(404)
    })

    test("3. Actualizar una persona (y retorna la persona actualizada)", async () => {
        const personas = { id: 2, nombre: "JuanXX", apellido: "Perez", edad: 30 }

        const res = await request(app).put("/personas").send(personas)
        console.log(">>>>",res.body)
        expect(res.body.borrada).toBeDefined()
        expect(res.body.actualizado).toBeDefined()

        expect(res.body.nombre).toBe(personas.nombre)
    })
})
...
describe("Borrar una persona", () => {
    ...
    test("3. Borrar una persona (y retorna la persona borrada)", async () => {
        const personas = { id: 3, borrada: true }

        const res = await request(app).put("/personas").send(personas)
        expect(res.body.borrada).toBe(true)
        expect(res.body.actualizado).toBeDefined()
        expect(res.body.id).toBe(personas.id)
    })

    test("4. Borrar una persona (debe haber uno menos)", async () => {
        const personas = { id: 4, borrada: true }

        const leerAntes = await request(app).get("/personas").send()
        const antes = leerAntes.body.length

        const borrar = await request(app).put("/personas").send(personas)
        expect(borrar.body.id).toBe(personas.id)
